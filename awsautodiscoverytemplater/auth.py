...
__author__ = 'drew.sonne@gmail.com'
...
class AWSArgumentParser(argparse.ArgumentParser):
    ...
    >>> aws_options = auth.AWSArgumentParser(default_role_session_name='elasticsearch_creation')
    ...
    optional arguments:
      ...
      --max-instances MAX_INSTANCES
    """
    def __init__(self, default_role_session_name):

        super(AWSArgumentParser, self).__init__(add_help=False)
...
class Credentials(object):
    ...
    The general usage is the following form:
        ...
        >>> credentials = auth.Credentials(**kwargs) # **kwargs passed from auth.get_arg_parser
...
def default_session(region=None):
    """
    A default session, without roles, or provided credentials.
    :param region: Region name as a string
    :return: default session, based on the environment
